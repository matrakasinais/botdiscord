...
# ============================================================
TOKEN        = os.environ["MTUxMjI5MDIyMDEwNjcxNTIxNg.GbReDX.v0SuUhUo9Qx_2cWVnCpODWQn16uYA6YykQBsTQ"]               # Definir no Railway / .env local
...
async def setup_canais(interaction: discord.Interaction):
    ...
    if ch:
        ...
        e = discord.Embed(title="⚡ BEM-VINDO AO GAMEBOOST PREMIUM!", description="O **otimizador mais completo** para Windows — feito para quem leva games a serio.\nO GameBoost aplica mais de **40 otimizacoes automaticas** no seu PC.", color=0xFF6D00)
    ...
    if ch:
        ...
        # Post de tweaks - SISTEMA
        t1 = discord.Embed(title="\u2699\ufe0f GUIA DE TWEAKS \u2014 SISTEMA", description="O que cada opcao faz e como impacta seu PC:", color=0xFF6D00)
        t1.add_field(name="\u2705 Habilitar ajustes de desempenho", value="Define prioridade do processador para apps em primeiro plano.\n**Impacto:** Jogos mais responsivos, menos microtravamentos.", inline=False)
        t1.add_field(name="\u2705 Remover atraso de menus", value="Zera o delay ao abrir menus do Windows (padrao 400ms).\n**Impacto:** Interface mais rapida e responsiva.", inline=False)
        t1.add_field(name="\u2705 Desativar limitacoes de rede", value="Remove o limite de banda reservada pelo Windows (10% padrao).\n**Impacto:** Menor ping, mais banda para jogos online.", inline=False)
        t1.add_field(name="\u2705 Desativar relatorio de erros", value="Desativa envio de relatorios de crash para a Microsoft.\n**Impacto:** Menos processos em segundo plano.", inline=False)
        t1.add_field(name="\u2705 Desativar assistente de compat.", value="Remove verificacoes de compatibilidade ao abrir programas.\n**Impacto:** Programas abrem mais rapido.", inline=False)
        t1.add_field(name="\u2705 Efeitos visuais minimos", value="Desativa animacoes e transparencias do Windows.\n**Impacto:** Libera CPU/GPU para jogos.", inline=False)
        t1.add_field(name="\u26a0\ufe0f Desativar SmartScreen", value="Remove verificacao de seguranca ao abrir arquivos.\n**Cuidado:** Menos protecao contra malware. Deixe desmarcado se nao souber o que esta fazendo.", inline=False)
        t1.add_field(name="\u2705 Carimbo NTFS off", value="Desativa atualizacao de data de acesso em arquivos.\n**Impacto:** Disco trabalha menos, leitura mais rapida.", inline=False)
        await ch.send(embed=t1)

        # Post de tweaks - PRIVACIDADE
        t2 = discord.Embed(title="\u2699\ufe0f GUIA DE TWEAKS \u2014 PRIVACIDADE", color=0xFF6D00)
        t2.add_field(name="\u2705 Desativar telemetria Microsoft", value="Para o envio de dados de uso para a Microsoft.\n**Impacto:** Mais privacidade, menos uso de rede em segundo plano.", inline=False)
        t2.add_field(name="\u2705 Desativar Cortana", value="Desativa o assistente virtual da Microsoft.\n**Impacto:** Libera RAM e CPU.", inline=False)
        t2.add_field(name="\u2705 Desativar noticias e interesses", value="Remove widget de noticias da barra de tarefas.\n**Impacto:** Menos uso de rede e CPU em segundo plano.", inline=False)
        t2.add_field(name="\u2705 Desativar anuncios menu iniciar", value="Remove sugestoes de apps no menu iniciar.\n**Impacto:** Menu iniciar mais limpo e rapido.", inline=False)
        await ch.send(embed=t2)

        # Post de tweaks - JOGOS
        t3 = discord.Embed(title="\u2699\ufe0f GUIA DE TWEAKS \u2014 JOGOS", color=0xFF6D00)
        t3.add_field(name="\u2705 Ativar modo de jogo Windows", value="Ativa o Game Mode nativo do Windows.\n**Impacto:** Windows prioriza recursos para o jogo ativo.", inline=False)
        t3.add_field(name="\u2705 Desativar Xbox Game Bar", value="Remove o overlay do Xbox (Win+G).\n**Impacto:** Menos uso de recursos, sem overlay indesejado.", inline=False)
        t3.add_field(name="\u2705 HAGS - GPU Scheduling", value="Ativa agendamento de GPU por hardware.\n**Impacto:** Menor latencia em jogos modernos. Em GPUs antigas pode causar instabilidade.", inline=False)
        t3.add_field(name="\u2705 Desativar MPO (anti-stutter)", value="Desativa Multi-Plane Overlay.\n**Impacto:** Elimina stuttering e tearing. Muito recomendado!", inline=False)
        t3.add_field(name="\u2705 Prioridade GPU maxima", value="Define prioridade maxima para processos de GPU em jogos.\n**Impacto:** GPU focada 100% nos jogos.", inline=False)
        await ch.send(embed=t3)

        # Post de tweaks - GPU
        t4 = discord.Embed(title="\u2699\ufe0f GUIA DE TWEAKS \u2014 GPU NVIDIA & AMD", color=0xFF6D00)
        t4.add_field(name="\u2705 PowerMizer max (NVIDIA)", value="Forca GPU NVIDIA a rodar sempre na frequencia maxima.\n**Impacto:** FPS mais estavel, sem throttling de clock.", inline=False)
        t4.add_field(name="\u2705 Threaded Optimization (NVIDIA)", value="Permite CPU usar multiplos threads para comandos a GPU.\n**Impacto:** Melhor aproveitamento de CPUs multi-core.", inline=False)
        t4.add_field(name="\u2705 Desativar ULPS (AMD)", value="Desativa Ultra Low Power State da GPU AMD.\n**Impacto:** GPU sempre pronta, sem delay ao iniciar carga grafica.", inline=False)
        t4.add_field(name="\u2705 Desativar thermal throttle (AMD)", value="Remove limitacao de clock por temperatura em GPUs AMD.\n**Impacto:** FPS mais estavel. Certifique que o cooler funciona bem.", inline=False)
        await ch.send(embed=t4)

        # Post de tweaks - CPU/RAM
        t5 = discord.Embed(title="\u2699\ufe0f GUIA DE TWEAKS \u2014 CPU / RAM", color=0xFF6D00)
        t5.add_field(name="\u2705 Desativar Core Parking", value="Impede o Windows de desligar nucleos da CPU quando ociosa.\n**Impacto:** Todos os nucleos disponiveis imediatamente.", inline=False)
        t5.add_field(name="\u2705 SystemResponsiveness = 0", value="Prioriza 100% dos recursos de multimedia para jogos.\n**Impacto:** Jogos recebem maxima prioridade de CPU.", inline=False)
        t5.add_field(name="\u2705 Timer de alta resolucao", value="Ativa timer de sistema de alta precisao.\n**Impacto:** Timing dos jogos mais preciso, FPS mais estavel.", inline=False)
        t5.add_field(name="\u2705 Desativar paginacao executiva", value="Mantem codigo do kernel na RAM ao inves do disco.\n**Impacto:** Menos stuttering, sistema mais responsivo.", inline=False)
        t5.add_field(name="\u2705 CPU 100% sem throttling", value="Remove limitacoes de frequencia da CPU pelo plano de energia.\n**Impacto:** CPU sempre na frequencia maxima.", inline=False)
        t5.add_field(name="\u2705 Ultimate Performance", value="Ativa o plano de energia mais agressivo do Windows.\n**Impacto:** Maximo desempenho. Recomendado para desktops.", inline=False)
        await ch.send(embed=t5)

        # Post de tweaks - LIMPEZA
        t6 = discord.Embed(title="\u2699\ufe0f GUIA DE TWEAKS \u2014 LIMPEZA", color=0xFF6D00)
        t6.add_field(name="\u2705 Limpar arquivos temporarios", value="Remove arquivos temp do Windows.\n**Impacto:** Libera espaco em disco, sistema mais limpo.", inline=False)
        t6.add_field(name="\u2705 Limpar Prefetch", value="Remove cache de pre-carregamento de aplicativos.\n**Impacto:** Libera espaco. O Windows recria automaticamente.", inline=False)
        t6.add_field(name="\u2705 Limpar cache DNS", value="Reseta o cache de enderecos de internet.\n**Impacto:** Resolve problemas de conexao, menor ping em alguns casos.", inline=False)
        await ch.send(embed=t6)
...
@tree.command(name="setup_atualizacoes", description="[ADMIN] Posta o historico de atualizacoes no canal novidades")
async def setup_atualizacoes(interaction: discord.Interaction):
    if interaction.user.id != ADMIN_ID:
        return await interaction.response.send_message("Sem permissao.", ephemeral=True)
    ...
    ch = None
    for c in interaction.guild.text_channels:
        if "novidades" in c.name.lower():
            ch = c
            break

    if not ch:
        return await interaction.followup.send("Canal novidades nao encontrado.", ephemeral=True)

    e0 = discord.Embed(
        title="📋 HISTORICO DE ATUALIZACOES — GAMEBOOST PREMIUM",
        description="Acompanhe tudo que foi melhorado no GameBoost Premium desde o lancamento!\nAtualizamos constantemente para garantir a melhor experiencia.",
        color=0xFF6D00
    )
    e0.set_footer(text="GameBoost Premium • Criado por matraka")
    await ch.send(embed=e0)

    e3 = discord.Embed(title="⚡ v1.3 — Interface Profissional", color=0xFF6D00)
    e3.add_field(name="O que mudou", value=(
        "• Aba **TWEAKS** reformulada com visual moderno\n"
        "• Categorias com acordeao — clique para expandir/fechar\n"
        "• Tooltip ao passar o mouse em cada opcao\n"
        "• Scrollbar no tema laranja em todas as abas\n"
        "• Botao direto para o **Discord** na tela de ativacao\n"
        "• Hover effects e cursor de mao nos botoes"
    ), inline=False)
    e3.set_footer(text="Atualizacao mais recente")
    await ch.send(embed=e3)

    e2 = discord.Embed(title="🔧 v1.2 — Melhorias Visuais e Correcoes", color=0xFF6D00)
    e2.add_field(name="O que mudou", value=(
        "• Rodape Criado por matraka em todas as paginas\n"
        "• Data de expiracao em formato brasileiro DD/MM/AAAA\n"
        "• Menu reorganizado: BOOST | RELATORIO | TWEAKS | RAM VIRTUAL | PERFIL\n"
        "• Botao versao gratuita com confirmacao\n"
        "• Correcoes de bugs na tela de ativacao"
    ), inline=False)
    await ch.send(embed=e2)

    e1 = discord.Embed(title="🚀 v1.1 — Sistema de Licencas e Bot Discord", color=0xFF6D00)
    e1.add_field(name="O que mudou", value=(
        "• Bot Discord com sistema de licencas automatico\n"
        "• Chave e arquivo enviados por DM apos compra\n"
        "• Vinculacao de licenca ao PC via HWID\n"
        "• Versao gratuita separada da premium\n"
        "• Correcao do problema de FPS do timer do sistema"
    ), inline=False)
    await ch.send(embed=e1)

    e0b = discord.Embed(title="🎮 v1.0 — Lancamento Inicial", color=0xFF6D00)
    e0b.add_field(name="Funcionalidades lancadas", value=(
        "• BOOST com 40+ otimizacoes automaticas\n"
        "• RELATORIO com monitor de CPU, RAM, GPU e Disco\n"
        "• TWEAKS com ajustes individuais\n"
        "• RAM VIRTUAL para PCs com pouca memoria\n"
        "• PERFIL com informacoes da licenca"
    ), inline=False)
    await ch.send(embed=e0b)

    enext = discord.Embed(
        title="🔮 Proximas Atualizacoes",
        description="Estamos sempre trabalhando para melhorar o GameBoost Premium!\n\nTem sugestoes? Abra um ticket em **#abrir-ticket**!",
        color=0x646478
    )
    enext.set_footer(text="GameBoost Premium • Obrigado pelo suporte!")
    await ch.send(embed=enext)

    await interaction.followup.send("✅ Historico de atualizacoes postado!", ephemeral=True)
